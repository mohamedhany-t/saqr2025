...
هذه هي الكيانات الرئيسية في قاعدة البيانات (NoSQL Database):
...
- **الوظائف:**
  - **إدارة المستخدمين:**
    ...
    - **الأثر:** يتم إنشاء حساب للمستخدم في نظام المصادقة، وسجل في `users`، وسجل مطابق في `couriers` أو `companies`، وسجل في مجموعة الصلاحيات (`roles_*`).
  - **إدارة الشحنات:**
    - **الزر:** "شحنة جديدة".
